...
class GameScene extends Phaser.Scene {
    ...
    showBaitWarning(text) {
        const cx = this.cameras.main.width / 2;
        const container = this.add.container(cx, 380);
        
        const bg = this.add.rectangle(0, 0, 320, 120, 0x000000, 0.95);
        bg.setStrokeStyle(4, 0xFFAA00);
        container.add(bg);

        container.add(this.add.text(0, 0, text, {
            fontSize: '26px', fontFamily: 'Microsoft YaHei',
            color: '#FFAA00', align: 'center'
        }).setOrigin(0.5));

        container.add(this.add.text(0, 45, '请点击【配置】设置鱼饵', {
